...
<xsl:template match= "/Cheader|/root/Cheader">
  ...
  <xsl:text>
package </xsl:text><xsl:value-of select="$outPackage" /><xsl:text> ;

import org.vishia.byteData.*;

public class </xsl:text><xsl:value-of select="$outFile" /><xsl:text>
{
</xsl:text>
  <xsl:for-each select="outside/enumDefinition">
    <xsl:text>

  //enum </xsl:text><xsl:value-of select="@name" /><xsl:text>:
  //( ?call enumConstantDefinition:"enumElement"()?)
  </xsl:text><xsl:for-each select="enumElement"><xsl:call-template name="defineToStaticInt"></xsl:call-template></xsl:for-each><xsl:text>
  ...
  {
    </xsl:text><xsl:for-each select="attribute[count(arraysize/@value)>0]"><xsl:call-template name="arraySizesOfAttribute"></xsl:call-template></xsl:for-each><xsl:text>
    ...
    </xsl:text><xsl:for-each select="xxxenumDefinition/enumElement"><xsl:call-template name="enumConstantDefinition"></xsl:call-template></xsl:for-each><xsl:text>
    </xsl:text><xsl:for-each select="enumDefinition/enumElement"><xsl:call-template name="defineToStaticInt"></xsl:call-template></xsl:for-each><xsl:text>
    </xsl:text><xsl:for-each select="defineDefinition"><xsl:call-template name="defineToStaticInt"></xsl:call-template></xsl:for-each><xsl:text>

    </xsl:text>
    <!-- enum and define definition at level of CLASS_C -->
    <xsl:if test="local-name(..)='CLASS_C'">
      <xsl:variable name="classSuffix"><xsl:text>_</xsl:text><xsl:value-of select="../@name" /><xsl:text></xsl:text></xsl:variable>
      <xsl:text>
      </xsl:text><xsl:for-each select="../enumDefinition/enumElement"><xsl:call-template name="defineToStaticInt"><xsl:with-param name="classSuffix"  select="$classSuffix" /></xsl:call-template></xsl:for-each><xsl:text>
      </xsl:text><xsl:for-each select="../defineDefinition"><xsl:call-template name="defineToStaticInt"><xsl:with-param name="classSuffix"  select="../@name" /></xsl:call-template></xsl:for-each><xsl:text>
      </xsl:text>
    </xsl:if>
    ...
    <xsl:for-each select="attribute[1]">
      <xsl:variable name="typetest"><xsl:text>(</xsl:text><xsl:value-of select="type/@name" /><xsl:text>)</xsl:text></xsl:variable>
    ...
    <!-- NOTE: generate setBigEndian for all classes, elsewhere super.setBigEndian may be not work if it is a derivated class. -->
      ...
      <xsl:for-each select="attribute">
        ...
        </xsl:text><xsl:value-of select="@name" /><xsl:text>.assignData(super.data, super.idxEnd, super.idxBegin + kIdx</xsl:text><xsl:value-of select="@name" /><xsl:text>);  //embedded structure
        </xsl:text><xsl:value-of select="@name" /><xsl:text>.setBigEndian(super.bBigEndian);</xsl:text>
        </xsl:if>
      ...
      <xsl:for-each select="attribute">
        ...
        <xsl:if test="not(boolean(variante)) and not(boolean(type/@pointer)) and not(boolean(type/@constPointer)) and not(contains('(int)(float)(short)(char)(uint)(ushort)(byte)(int8)(int16)(int32)(uint8)(uint16)(uint32)(Int8)(Int16)(Int32)(UInt8)(UInt16)(UInt32)(int16BigEndian)(int32BigEndian)(int64BigEndian)(floatBigEndian)(doubleBigEndian)(ptrBigEndian)',$typetest))">
          ...
          <xsl:text>
        </xsl:text><xsl:value-of select="@name" /><xsl:text>.setBigEndian(val);  //embedded structure
          </xsl:text>
        </xsl:if>
  ...
  <xsl:template name="arraySizesOfAttribute">
    <xsl:text>
    protected static final int kSize</xsl:text><xsl:value-of select="@name" /><xsl:text> = </xsl:text><xsl:value-of select="arraysize/@value" /><xsl:text>;
    </xsl:text>
  </xsl:template>
  ...
  <xsl:template name="enumConstantDefinition">
    ...
    <xsl:text>
    public static final int </xsl:text><xsl:value-of select="@name" /><xsl:text> = <!-- ??"count(hexnumber)>0"?'0x' --></xsl:text><xsl:value-of select="hexnumber" /><xsl:text></xsl:text><xsl:value-of select="number" /><xsl:text></xsl:text><xsl:value-of select="intnumber" /><xsl:text>;
    </xsl:text>
	</xsl:template>
...
<xsl:param name="sizeMax" select="'0'" />
  <xsl:text>/*TEST </xsl:text><xsl:value-of select="local-name(*)" /><xsl:text>-</xsl:text><xsl:value-of select="*/type/@name" /><xsl:text>*/ </xsl:text>
  ...
  <xsl:text>/*</xsl:text><xsl:value-of select="local-name(*)" /><xsl:text>-</xsl:text><xsl:value-of select="*/@name" /><xsl:text>=</xsl:text><xsl:value-of select="$nrofBytes" /><xsl:text> (</xsl:text><xsl:value-of select="$sizeMaxNew" /><xsl:text>) */ </xsl:text>
  <xsl:choose>
  ...
  <xsl:template name="gensetMethod">
    ...
    { addChildString(val);  //The last element is designated with '@lastUndefsizeArray' in Header-file, therefore the element is added as child,
        ...
        <xsl:text>
    public void set_</xsl:text><xsl:value-of select="@name" /><xsl:text>(String val)
    { setString(kIdx</xsl:text><xsl:value-of select="@name" /><xsl:text>, </xsl:text><xsl:value-of select="arraysize/@value" /><xsl:text></xsl:text><xsl:value-of select="arraysize/@symbolValue" /><xsl:text>, val);
    }
	  </xsl:text>
	    ...
	    { //type of struct-attribut is </xsl:text><xsl:value-of select="type/@name" /><xsl:text>
	      ...
	      <xsl:otherwise>
	        <xsl:text>
	      //note: method to set </xsl:text><xsl:value-of select="@name" /><xsl:text>, not able to generate.
	        </xsl:text>
	      </xsl:otherwise></xsl:choose>
        ...
        <xsl:variable name="setMethod">
	      ...
	      <xsl:when test="$setMethod != '???'">
	        <xsl:text>
	    public void set_</xsl:text><xsl:value-of select="@name" /><xsl:text>(</xsl:text><xsl:value-of select="$valueType" /><xsl:text> val</xsl:text><xsl:choose><xsl:when test="count(arraysize)>0"><xsl:text>, int idx</xsl:text></xsl:when></xsl:choose><xsl:text>)
	    { //type of struct-attribut is </xsl:text><xsl:value-of select="type/@name" /><xsl:text>
	      </xsl:text><xsl:value-of select="$setMethod" /><xsl:text>(kIdx</xsl:text><xsl:value-of select="@name" /><xsl:text></xsl:text><xsl:choose><xsl:when test="count(arraysize)>0"><xsl:text>, idx, </xsl:text><xsl:value-of select="arraysize/@value" /><xsl:text></xsl:text><xsl:value-of select="arraysize/@symbolValue" /><xsl:text></xsl:text></xsl:when></xsl:choose><xsl:text>, val);
	    }
	      </xsl:text>
	      </xsl:when>
	      <xsl:otherwise>
  ...
  <xsl:template name="gengetMethod">
    ...
    <xsl:choose><xsl:when test="type/@name='char' and count(arraysize)>0">
      <xsl:text>
      public String get_</xsl:text><xsl:value-of select="@name" /><xsl:text>(String val)
      { return getString(kIdx</xsl:text><xsl:value-of select="@name" /><xsl:text>, </xsl:text><xsl:value-of select="arraysize/@value" /><xsl:text></xsl:text><xsl:value-of select="arraysize/@symbolValue" /><xsl:text>);
      }
      </xsl:text>
    </xsl:when>
    ...
    <xsl:when test="boolean(type)">
      ...
      <xsl:when test="$getMethod != '???'">
        ...
        <xsl:text>
      public </xsl:text><xsl:value-of select="$valueType" /><xsl:text> get_</xsl:text><xsl:value-of select="@name" /><xsl:text>(</xsl:text><xsl:choose><xsl:when test="count(arraysize)>0"><xsl:text>int idx</xsl:text></xsl:when></xsl:choose><xsl:text>)
      { //type of struct-attribut is </xsl:text><xsl:value-of select="type/@name" /><xsl:text>
        return </xsl:text><xsl:value-of select="$getMethod" /><xsl:text>(kIdx</xsl:text><xsl:value-of select="$nameIdx" /><xsl:text></xsl:text><xsl:choose><xsl:when test="count(arraysize)>0"><xsl:text>, idx, </xsl:text><xsl:value-of select="arraysize/@value" /><xsl:text></xsl:text><xsl:value-of select="arraysize/@symbolValue" /><xsl:text></xsl:text></xsl:when></xsl:choose><xsl:text>);
      }
        </xsl:text>
        <xsl:if test="boolean(arraysize)">
          <xsl:text>
      public final static int size_</xsl:text><xsl:value-of select="@name" /><xsl:text> = </xsl:text><xsl:value-of select="arraysize/@value" /><xsl:text></xsl:text><xsl:value-of select="arraysize/@symbolValue" /><xsl:text>;
          </xsl:text>
        </xsl:if>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:text>
	...
	<xsl:template match= "enumValue/hexnumber"><xsl:text> 0x</xsl:text><xsl:call-template name="toHex"><xsl:with-param name="decimalNumber"  select="hexnumber" /></xsl:call-template><xsl:text></xsl:text></xsl:template>
...
<xsl:param name="classSuffix" select="'-'" />
  ...
  <xsl:choose>
    <xsl:when test="boolean(intvalue) or boolean(intnumber)">
      <xsl:text>
    public static final int </xsl:text><xsl:value-of select="$name" /><xsl:text> = </xsl:text><xsl:value-of select="intvalue" /><xsl:text></xsl:text><xsl:value-of select="intnumber" /><xsl:text>;</xsl:text>
    </xsl:when>
    <xsl:when test="boolean(hexvalue)">
      <!-- xsl:text>
    public static final int </xsl:text><xsl:value-of select="$name" /><xsl:text> = </xsl:text><xsl:value-of select="hexvalue" /><xsl:text>;</xsl:text -->
      <xsl:text>
    public static final int </xsl:text><xsl:value-of select="$name" /><xsl:text> = 0x</xsl:text><xsl:call-template name="toHex"><xsl:with-param name="decimalNumber"  select="hexvalue" /></xsl:call-template><xsl:text>;</xsl:text>
    </xsl:when>
    <xsl:when test="boolean(hexnumber)">
      <xsl:text>
    public static final int </xsl:text><xsl:value-of select="$name" /><xsl:text> = 0x</xsl:text><xsl:call-template name="toHex"><xsl:with-param name="decimalNumber"  select="hexnumber" /></xsl:call-template><xsl:text>;</xsl:text>
    </xsl:when>
    <xsl:when test="boolean(stringvalue)">
      <xsl:text>
    public static final String </xsl:text><xsl:value-of select="$name" /><xsl:text> = "</xsl:text><xsl:value-of select="stringvalue" /><xsl:text>";</xsl:text>
    </xsl:when>
    <xsl:otherwise>
      <xsl:text>
    //#define </xsl:text><xsl:value-of select="$name" /><xsl:text> </xsl:text><xsl:value-of select="value" /><xsl:text></xsl:text>
    </xsl:otherwise>
  </xsl:choose>
  <xsl:text>    //Cheader2ByteDataAccess_Java.xslp: classSuffix=:</xsl:text><xsl:value-of select="$classSuffix" /><xsl:text>:</xsl:text>
</xsl:template>
